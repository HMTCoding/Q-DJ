  ...
  const fetchCurrentState = async () => {
    try {
      const response = await fetch(
        `/api/spotify/current-state?eventId=${eventId}`
      );
  ...
  return (
    <div className="relative min-h-screen w-full bg-black overflow-hidden">
      ...
      <div className="absolute top-8 right-8 z-20">
        ...
        <p className="text-white text-center mt-2 text-sm bg-black/50 px-2 py-1 rounded">
          Scan to join
        </p>
      </div>
          ...
          {state?.message ? (
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-12">
              ...
              <div className="flex-shrink-0">
                <img
                  src={
                    state.currentlyPlaying.album.images[0]?.url ||
                  ...
                  onError={(e) => {
                    ...
                    target.src =
                      "https://placehold.co/400x400/1a1a1a/FFFFFF?text=No+Image";
                  }}
                />
              </div>
              ...
              <div className="flex flex-col items-center md:items-start text-center md:text-left">
                ...
                <p className="text-3xl md:text-4xl text-gray-300 mb-8">
                  {state.currentlyPlaying.artists
          ...
          {state?.queue && state.queue.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {state.queue.map((track, index) => (
                ...
                >
                  <span className="text-2xl font-bold mr-4 text-gray-300">
                    #{index + 1}
                  </span>
                  <img
                    ...
                    onError={(e) => {
                      const target = e.target as HTMLImageElement;
                      target.src =
                        "https://placehold.co/300x300/1a1a1a/FFFFFF?text=No+Image";
                    }}
                  />
